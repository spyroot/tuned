...
class BootloaderPlugin(base.Plugin):
    ...
    *TuneD* profile `profile_1`:
    ...
    def _rpm_ostree_kargs(self, append=None, delete=None):
        """
    ...
    def _bls_enabled(self):
        ...
        return re.search(
            r"^\s*GRUB_ENABLE_BLSCFG\s*=\s*\"?\s*[tT][rR][uU][eE]\s*\"?\s*$",
            grub2_default_env,
            flags=re.MULTILINE
        ) is not None
    ...
    def _remove_grub2_tuning(self):
        ...
        for f in self._grub2_cfg_file_names:
            self._cmd.add_modify_option_in_file(
                f,
                {
                    "set\s+" + consts.GRUB2_TUNED_VAR: "",
                    "set\s+" + consts.GRUB2_TUNED_INITRD_VAR: ""
                },
                add=False
            )
    ...
    def _grub2_cfg_patch_initial(self, grub2_cfg, d):
        ...
        for i in d2:
            ...
            grub2_cfg = re.sub(
                r"^(\s*" + i + r"(?:16|efi)?\s+\S+rescue.*)\$" + d2[i] + r" *(.*)$", r"\1\2", grub2_cfg,
                flags=re.MULTILINE
            )
    ...
    def _grub2_default_env_patch(self):
        ...
        d = {
            "GRUB_CMDLINE_LINUX_DEFAULT": consts.GRUB2_TUNED_VAR,
    ...
    def _grub2_default_env_unpatch(self):
        ...
        write = False
        if re.search(
                r"^GRUB_CMDLINE_LINUX_DEFAULT=\"\$\{GRUB_CMDLINE_LINUX_DEFAULT:\+\$GRUB_CMDLINE_LINUX_DEFAULT \}\\\$" +
    ...
    def _grub2_cfg_patch(self, d):
        ...
        for f in self._grub2_cfg_file_names:
            ...
            patch_initial = False
            for opt in d:
                (grub2_cfg_new, nsubs) = re.subn(
                    r"\b(set\s+" + opt + "\s*=).*$",
                    r"\1" + "\"" + self._cmd.escape(d[opt]) + "\"", grub2_cfg_new,
                    flags=re.MULTILINE
                )
                if nsubs < 1 or re.search(
                        r"\$" + opt, grub2_cfg, flags=re.MULTILINE) is None:
                    patch_initial = True
