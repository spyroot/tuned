...
class ModulesPlugin(base.Plugin):
    ...
    def _instance_apply_static(self, instance):
        ...
        skip_check = False
        reload_list = []
        for option, value in list(instance.get_modules().items()):
    ...
    def _instance_verify_static(self, instance, ignore_missing, devices):
        ret = True
        # not all modules exports all their parameters through sysfs, so hardcode check with ignore_missing
        ignore_missing = True
        r = re.compile(r"\s+")
        for option, value in list(instance.get_modules.items()):
            module = self._variables.expand(option)
                ...
                for item in l:
                    ...
                    if len(arg) != 2:
                        ...
                        if not self._verify_value(
                                arg[0], arg[1],
                                self._cmd.read_file(
                                    mpath + "/parameters/" + ModulesPlugin._unquote_path(arg[0]),
                                    err_ret=None,
                                    no_error=True),
                                ignore_missing):
                            ret = False
