...
class MountsPlugin(base.Plugin):
    ...
    def _generate_mountpoint_topology(cls):
        ...
        for columns in [line.split() for line in stdout.splitlines()]:
            ...
            mountpoint_topology.setdefault(mountpoint,
                                           {"disks": set(), "device_name":
                                               device_name, "filesystem": filesystem})
    ...
    @staticmethod
    def _get_device_cache_type(device):
        """
    ...
    def _mountpoint_has_writeback_cache(self, mountpoint):
        ...
        for device in self._mountpoint_topology[mountpoint]["disks"]:
            if MountsPlugin._get_device_cache_type(device) == "write back":
                return True
        return False

    @staticmethod
    def _mountpoint_has_barriers(mountpoint):
        """
    ...
    @staticmethod
    def _remount_partition(partition, options):
        """
        Remounts partition.
        """
    ...
    def _disable_barriers(self, start, value, mountpoint, verify):
        ...
        if start:
            ...
            if not self._mountpoint_topology[mountpoint]["filesystem"].startswith("ext"):
                ...
                original_value = MountsPlugin._mountpoint_has_barriers(mountpoint)
            ...
            MountsPlugin._remount_partition(mountpoint, "barrier=0")
            ...
            MountsPlugin._remount_partition(mountpoint, "barrier=1")
