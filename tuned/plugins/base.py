...
class Plugin(object):
    ...
    def instance_apply_tuning(self, instance):
        ...
        if instance.has_static_tuning:
            ...
            self._call_device_script(instance, instance.script_post,
                                     "apply", instance.assigned_devices)
        if instance.has_dynamic_tuning and self._global_cfg.get(consts.CFG_DYNAMIC_TUNING,
    ...
    def instance_verify_tuning(self, instance, ignore_missing):
        ...
        if instance.has_static_tuning:
            if not self._call_device_script(instance, instance.script_pre, "verify", devices):
                return False
            if not self._instance_verify_static(instance, ignore_missing, devices):
                return False
            if not self._call_device_script(instance, instance.script_post, "verify", devices):
                return False
            return True
        else:
            return None
    ...
    def instance_unapply_tuning(self, instance, full_rollback=False):
        ...
        if instance.has_static_tuning:
            ...
            self._call_device_script(instance, instance.script_pre, "unapply", instance.processed_devices,
                                     full_rollback=full_rollback)
    ...
    def _instance_apply_dynamic(self, instance, device):
        for option in [opt for opt in self._options_used_by_dynamic if
                       self._storage_get(instance, self._commands[opt], device) is None]:
    ...
    def _autoregister_commands(self):
        ...
        self._commands = collections.OrderedDict(
            sorted(iter(self._commands.items()), key=lambda name_info: name_info[1]["priority"]))
    ...
    def _process_assignment_modifiers(self, new_value, current_value):
        if new_value is not None:
            ...
            try:
                ...
                log.warn(
                    "cannot compare new value '%s' with current value '%s' by operator '%s', using '%s' directly as "
    ...
    def _log_verification_result(self, name, success, new_value,
        ...
        if success:
            ...
            if device is None:
                log.error(
                    consts.STR_VERIFY_PROFILE_VALUE_FAIL % (name, str(current_value).strip(), str(new_value).strip()))
            else:
    ...
    def _cleanup_all_non_device_commands(self, instance):
        for command in reversed([command for command in list(self._commands.values()) if not command["per_device"]]):
            if (instance.options.get(command["name"], None) is not None) or (
                    command["name"] in self._options_used_by_dynamic):
