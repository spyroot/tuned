...
class NetTuningPlugin(base.Plugin):
    ...
    def _instance_update_dynamic(self, instance, device):
        ...
        if device not in instance._stats:
    ...
    @staticmethod
    def _parse_config_parameters(value, context):
    ...
    @classmethod
    def _nf_conntrack_hashsize_path(cls):
    ...
    def _call_ip_link(self, args=None):
    ...
    @staticmethod
    def _replace_channels_parameters(context, params_list, dev_params):
        mod_params_list = []
    ...
    @staticmethod
    def _check_device_support(context, parameters, device, dev_params):
    ...
    def _set_device_parameters(self, context, value, device, sim,
        ...
        if dev_params:
            NetTuningPlugin._check_device_support(context, d, device, dev_params)
            # replace the channel parameters based on the device support
            if context == "channels" and str(dev_params[next(iter(d))]) in ["n/a", "0"]:
                d = NetTuningPlugin._replace_channels_parameters(context, self._cmd.dict2list(d), dev_params)
