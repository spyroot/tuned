...
class CPULatencyPlugin(base.Plugin):
    ...
    *TuneD* will try to set EPB to 'powersave'. If that fails, it will
    ...
    def _check_energy_perf_bias(self):
        ...
        retcode, out = self._cmd.execute(["x86_energy_perf_policy", "-r"],
                                         no_errors=[errno.ENOENT, retcode_unsupported])
    ...
    def _get_available_governors(self, device):
        return self._cmd.read_file(
            "/sys/devices/system/cpu/%s/cpufreq/scaling_available_governors" % device).strip().split()
    ...
    def _get_governor(self, device, ignore_missing=False):
        ...
        if not self._check_cpu_can_change_governor(device):
            return None
        data = self._cmd.read_file("/sys/devices/system/cpu/%s/cpufreq/scaling_governor" % device,
                                   no_error=ignore_missing).strip()
        if len(data) > 0:
            governor = data
    ...
    def _set_sampling_down_factor(self, sampling_down_factor, device, sim):
        ...
        if governor not in list(self._governors_map.values()):
            ...
            if not os.path.exists(path):
                log.debug("ignoring sampling_down_factor setting for CPU '%s', governor '%s' doesn't support it" % (
                    device, governor))
                return None
