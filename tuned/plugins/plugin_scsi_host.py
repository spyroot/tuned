...
class SCSIHostPlugin(hotplug.Plugin):
    ...
    @staticmethod
    def _get_alpm_policy_file(device):
    ...
    def _set_alpm(self, policy, device, sim):
        if policy is None:
            return None
        policy_file = SCSIHostPlugin._get_alpm_policy_file(device)
        if not sim:
            if os.path.exists(policy_file):
                self._cmd.write_to_file(policy_file, policy)
            else:
                log.info(
                    "ALPM control file ('%s') not found, skipping ALPM setting for '%s'" % (policy_file, str(device)))
                return None
        return policy
