...
class SchedulerUtilsSchedutils(SchedulerUtils):
    ...
    def __init__(self):
        ...
        super().__init__()
        self._dict_schedcfg2num = dict(
            (k, getattr(schedutils, name)) for k, name in self._dict_schedcfg2schedconst.items())
        # { schedutils.SCHED_FIFO: "SCHED_FIFO"... }
        self._dict_num2schedconst = dict(
            (getattr(schedutils, name), name) for name in self._dict_schedcfg2schedconst.values())
...
class SchedulerPlugin(base.Plugin):
    ...
    *TuneD* will create (and remove) the cgroup mountpoint. It implies
    ...
    @staticmethod
    def _calc_mmap_pages(mmap_pages):
    ...
    def _instance_init(self, instance):
        ...
        self._cgroups = collections.OrderedDict(
            [(SchedulerPlugin._sanitize_cgroup_path(option[7:]), self._variables.expand(affinity))
        ...
        if perf_mmap_pages == 0:
            log.error(
                "Invalid 'perf_mmap_pages' value specified: '%s', using default kernel value" % perf_mmap_pages_raw)
    ...
    def get_processes():
        ...
        for proc in ps.values():
            try:
                cmd = SchedulerPlugin._get_cmdline(proc)
                pid = proc["pid"]
                processes[pid] = cmd
                if "threads" in proc:
    ...
    def _tune_process_rt(self, pid, sched, prio):
        ...
        try:
            ...
            if hasattr(e, "errno") and e.errno == errno.ESRCH:
                ...
                log.error(
                    "Refusing to set scheduler and priority of PID %d, "
                    "reading original scheduling parameters failed: %s"
                    % (pid, e))
        return cont

    @staticmethod
    def _is_cgroup_affinity(affinity):
    ...
    def _get_cgroup_affinity(self, pid):
        ...
        # can be ambiguous
        for line in self._cmd.read_file(
                "%s/%s/%s" % (
                        consts.PROCFS_MOUNT_POINT, str(pid), "cgroup"),
                no_error=True).split("\n"):
            try:
                cgroup = line.split(":cpuset:")[1][1:]
    ...
    def _set_cgroup(self, pid, cgroup):
        cgroup = SchedulerPlugin._sanitize_cgroup_path(cgroup)
    ...
    @staticmethod
    def _parse_cgroup_affinity(cgroup):
        # "cgroup.CGROUP"
        cgroup = cgroup[7:]
    ...
    def _tune_process_affinity(self, pid, affinity, intersect=False):
        ...
        if affinity is None:
            return cont
        try:
            (is_cgroup, cgroup) = SchedulerPlugin._parse_cgroup_affinity(affinity)
            if is_cgroup:
                ...
                if intersect:
                    affinity = SchedulerPlugin._get_intersect_affinity(
                        prev_affinity, affinity, affinity)
                self._set_affinity(pid, affinity)
            self._store_orig_process_affinity(
                pid, prev_affinity, is_cgroup)
        except (SystemError, OSError) as e:
            if hasattr(e, "errno") and e.errno == errno.ESRCH:
    ...
    def _convert_affinity(self, str_affinity):
        if str_affinity == "*":
            affinity = None
        elif SchedulerPlugin._is_cgroup_affinity(str_affinity):
    ...
    def _cgroup_create_group(self, cgroup):
        ...
        if (not self._cmd.write_to_file(
                "%s/%s" % (path, "cpuset.mems"),
                self._cmd.read_file(
                    "%s/%s" % (self._cgroup_mount_point, "cpuset.mems"),
                    no_error=True),
                no_error=True)):
            log.error("Unable to initialize 'cpuset.mems ' for cgroup '%s'" % path)

    def _cgroup_initialize_groups(self):
        if self._cgroup is not None and self._cgroup not in self._cgroups:
    ...
    def _cgroup_initialize(self):
        ...
        (ret, out) = self._cmd.execute(
            ["mount", "-t", "cgroup", "-o", "cpuset", "cpuset", self._cgroup_mount_point])
        if ret != 0:
            log.error("Unable to mount '%s'" % self._cgroup_mount_point)

    @staticmethod
    def _remove_dir(cgroup):
        try:
            os.rmdir(cgroup)
        except OSError as e:
    ...
    def _cgroup_finalize_groups(self):
        for cg in reversed(self._cgroups):
            SchedulerPlugin._remove_dir("%s/%s" % (self._cgroup_mount_point, cg))
        if self._cgroup is not None and self._cgroup not in self._cgroups:
            SchedulerPlugin._remove_dir("%s/%s" % (self._cgroup_mount_point, self._cgroup))
    ...
    def _cgroup_finalize(self):
        ...
        if ret != 0:
            ...
            return False
        SchedulerPlugin._remove_dir(self._cgroup_mount_point)
        d = os.path.dirname(self._cgroup_mount_point)
        if d != "/":
            SchedulerPlugin._remove_dir(d)
    ...
    def _cgroup_set_affinity(self):
        ...
        if self._affinity is not None and self._cgroup is not None and self._cgroup not in self._cgroups:
    ...
    def _instance_apply_static(self, instance):
        ...
        self._cgroup = SchedulerPlugin._sanitize_cgroup_path(self._variables.expand(
        ...
        try:
            ps = SchedulerPlugin.get_processes()
        except (OSError, IOError) as e:
    ...
    def _restore_ps_affinity(self):
        try:
            ...
            log.error("error unapplying tuning, "
                      "cannot get information about running processes: %s" % e)
            return
    ...
    def _cgroup_cleanup_tasks_one(self, cgroup):
        ...
        while data != "" and cnt > 0:
            ...
            if data not in ["", " "]:
                for sp in data.split("\n"):
                    self._cmd.write_to_file(
                        "%s/%s" % (self._cgroup_mount_point, "tasks"),
                        sp, no_error=True)
            cnt -= 1
        if cnt == 0:
    ...
    def _add_pid(self, instance, pid: int, r):
        try:
            ...
            if e.errno == errno.ENOENT \
                    or e.errno == errno.ESRCH:
                log.debug(
                    "Failed to get cmdline of PID %d, the task vanished." % pid)
            else:
    ...
    def _set_affinity(self, pid, affinity):
        ...
        # incorrectly raised SystemError instead of OSError
            ...
            if hasattr(e, "errno") and e.errno == errno.ESRCH:
                ...
                res = SchedulerPlugin._affinity_changeable(pid)
                if res == 1 or res == -2:
    ...
    @staticmethod
    def _get_intersect_affinity(affinity1, affinity2, affinity3):
    ...
    def _set_all_obj_affinity(self, objs, affinity, threads=False):
        psl = [v for v in objs if re.search(
            self._ps_whitelist, SchedulerPlugin._get_stat_comm(v)) is not None]
        if self._ps_blacklist != "":
            psl = [v for v in psl if re.search(
                self._ps_blacklist, SchedulerPlugin._get_stat_comm(v)) is None]
        if self._cgroup_ps_blacklist_re != "":
            psl = [v for v in psl if re.search(
                self._cgroup_ps_blacklist_re, SchedulerPlugin._get_stat_cgroup(v)) is None]
        psd = dict([(v.pid, v) for v in psl])
        for pid in psd:
            try:
                cmd = SchedulerPlugin._get_cmdline(psd[pid])
            except (OSError, IOError) as e:
                if e.errno == errno.ENOENT \
                        or e.errno == errno.ESRCH:
                    ...
                    log.error("Refusing to set affinity of PID %d, failed to get its cmdline: %s"
                              % (pid, e))
                continue
            cont = self._tune_process_affinity(
                pid, affinity, intersect=True)
            if not cont:
                continue
            if pid in self._scheduler_original:
                self._scheduler_original[pid].cmdline = cmd
    ...
    def _set_all_irq_affinity(self, affinity):
        ...
        for irq in irqs.keys():
            try:
                ...
                log.debug(
                    "Read affinity of IRQ '%s': '%s'"
                    % (irq, prev_affinity))
            except KeyError:
                continue
            _affinity = SchedulerPlugin._get_intersect_affinity(prev_affinity, affinity, affinity)
    ...
    @staticmethod
    def _verify_irq_affinity(irq_description, correct_affinity,
                             current_affinity):
    ...
    def _verify_all_irq_affinity(self, correct_affinity, ignore_missing):
        ...
        for irq in irqs.keys():
            ...
            try:
                ...
                if not SchedulerPlugin._verify_irq_affinity(
                        irq_description,
                        correct_affinity,
                        current_affinity):
                    res = False
        ...
        if self._default_irq_smp_affinity_value != "ignore" and not SchedulerPlugin._verify_irq_affinity(
                "default IRQ SMP affinity",
                current_affinity,
                correct_affinity
                if self._default_irq_smp_affinity_value == "calc"
                else self._default_irq_smp_affinity_value):
            res = False
        return res
