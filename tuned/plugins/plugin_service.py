...
class OpenRCHandler(InitHandler):
    ...
    @staticmethod
    def is_running(name):
        (ret_code, out) = cmd.execute(["rc-service", name, "status"], no_errors=[0])
        return ret_code == 0

    @staticmethod
    def is_enabled(name, runlevel):
        (ret_code, out) = cmd.execute(["rc-update", "show", runlevel])
...
class SystemdHandler(InitHandler):
    ...
    @staticmethod
    def is_running(name):
        (ret_code, out) = cmd.execute(["systemctl", "is-active", name], no_errors=[0])
        return ret_code == 0

    @staticmethod
    def is_enabled(name, runlevel):
        (ret_code, out) = cmd.execute(["systemctl", "is-enabled", name], no_errors=[0])
        status = out.strip()
...
class ServicePlugin(base.Plugin):
    ...
    def _check_cmd(command):
        (ret_code, out) = cmd.execute(command, no_errors=[0])
        return ret_code == 0
    ...
    @staticmethod
    def _parse_service_options(name, val):
    ...
    def _instance_init(self, instance):
        ...
        self._services = collections.OrderedDict([(option[8:], self._parse_service_options(
            option[8:],
            self._variables.expand(value))) for
                                                  option, value in instance.options.items()
    ...
    def services_original(self):
        return self._services_original

    def get_services_original(self, val):
        return self._services_original[val]

    def set_services_original(self, s, v):
        self._services_original[s] = v
    ...
    def _instance_apply_static(self, instance):
        ...
        for service in self._services.items():
            ...
            instance.set_services_original(service[0], Service(is_running, is_enabled, service[1].cfg_file, runlevel))
    ...
    def _instance_verify_static(self, instance, ignore_missing, devices):
        ...
        for service in self._services.items():
            ...
            if ret_cfg_verify:
                log.info(consts.STR_VERIFY_PROFILE_OK % "service '%s' configuration '%s' matches" % (
                    service[0], service[1].cfg_file))
            elif ret_cfg_verify is not None:
                log.error(consts.STR_VERIFY_PROFILE_FAIL % "service '%s' configuration '%s' differs" % (
                    service[0], service[1].cfg_file))
                ret = False
            else:
                log.info(consts.STR_VERIFY_PROFILE_VALUE_MISSING % "service '%s' configuration '%s'" % (
                    service[0], service[1].cfg_file))
            is_enabled = self._init_handler.is_enabled(service[0], runlevel)
            is_running = self._init_handler.is_running(service[0])
    ...
    def _instance_unapply_static(self, instance, full_rollback=False):
        for name, value in list(instance.services_original().items()):
            if value.cfg_file:
