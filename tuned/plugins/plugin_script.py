    ...
    ** `stop` when unloading the profile
    ...
    def _instance_init(self, instance):
        ...
        if instance.options["script"] is not None:
            ...
            instance.scripts = instance.options["script"]
        else:
            instance.scripts = []
    ...
    def _instance_apply_static(self, instance):
        ...
        self._call_scripts(instance.scripts, ["start"])

    def _instance_verify_static(self, instance, ignore_missing, devices):
        ret = True
        if not super(ScriptPlugin, self)._instance_verify_static(instance, ignore_missing, devices):
            ret = False
        args = ["verify"]
        if ignore_missing:
            args += ["ignore_missing"]
        if self._call_scripts(instance.scripts, args):
            log.info(consts.STR_VERIFY_PROFILE_OK % instance.scripts)
        else:
            log.error(consts.STR_VERIFY_PROFILE_FAIL % instance.scripts)
            ret = False
        return ret
    ...
    def _instance_unapply_static(self, instance, full_rollback=False):
        ...
        self._call_scripts(reversed(instance.scripts), args)
