...
class VideoPlugin(base.Plugin):
    ...
    * `dpm-perfomance`
    ...
    def _init_devices(self):
        ...
        for device in self._hardware_inventory.get_devices("drm").match_sys_name("card*").match_property("DEVTYPE",
                                                                                                         "drm_minor"):
    ...
    @staticmethod
    def _radeon_powersave_files(device):
        return {
    ...
    def _set_radeon_powersave(self, value, device, sim):
        sys_files = VideoPlugin._radeon_powersave_files(device)
