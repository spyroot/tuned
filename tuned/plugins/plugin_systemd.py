...
class SystemdPlugin(base.Plugin):
    ...
    def __init__(self, *args, **kwargs):
        if not os.path.isfile(consts.SYSTEMD_SYSTEM_CONF_FILE):
            raise exceptions.NotSupportedPluginException(
                "Required systemd '%s' configuration file not found, disabling plugin." % consts.SYSTEMD_SYSTEM_CONF_FILE)
    ...
    def _remove_systemd_tuning(self):
        ...
        if conf is not None:
            ...
            if cpu_affinity_saved is None:
                ...
                conf = SystemdPlugin._add_keyval(conf, consts.SYSTEMD_CPUAFFINITY_VAR, cpu_affinity_saved)
    ...
    def _cmdline(self, enabling, value, verify, ignore_missing):
        ...
        conf = self._read_systemd_system_conf()
        if conf is not None:
            conf_affinity = SystemdPlugin._get_keyval(conf, consts.SYSTEMD_CPUAFFINITY_VAR)
        ...
        if enabling:
            ...
            log.info("setting '%s' to '%s' in the '%s'" % (
            consts.SYSTEMD_CPUAFFINITY_VAR, v_unpacked, consts.SYSTEMD_SYSTEM_CONF_FILE))
            self._write_systemd_system_conf(SystemdPlugin._add_keyval(conf, consts.SYSTEMD_CPUAFFINITY_VAR, v_unpacked))
